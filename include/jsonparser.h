...
{ // TODO: Make efficient. This function is speed bottleneck.
...
// delimiter
        ...
        if (quote_stack.size() == 0)
        { // comment cannot start inside string
...
/* Use: RSJresource("RSJ_string_data").as<RSJobject>()["keyName"].as<RSJarray>()[2].as<int>()
    ...
    vector<string> get_keys(bool force = false);
...
// opertor[]
    ...
    { // specialized outside class declaration
...
// parse as object:
            ...
            { // a valid object
                ...
                for (int a = 0; a < nvPairs.size(); ++a)
                {
                    ...
                    keys.push_back(strip_outer_quotes(nvPairs[a].substr(0, assignmentPos)));
...
// parse as array
    ...
    int cleanup(void)
    {
        ...
        if (type == RSJ_ARRAY)
        { // erases only the non-existent elements at the tail
...
inline
void RSJresource::parse_full(bool force, int max_depth, int* parse_count_for_verbose_p)
...
inline
vector<string> RSJresource::get_keys(bool force)
{
    if (!parsed_data_p)  parsed_data_p = new RSJparsedData;
    if (parsed_data_p->type == RSJ_UNKNOWN || force)  parsed_data_p->parse(data, RSJ_OBJECT);
    return (parsed_data_p->keys);
}
...
{ // returns copy -- for being consistent with other 'as' specializations
    if (!exists()) return (def);
    vectorType ret;
    as_array();
    for (auto it = parsed_data_p->array.begin(); it != parsed_data_p->array.end(); ++it)
