...
void ThemeToggle::draw()
{
    ...
    for (int i : index_order)
    {
        ...
        if (i == selected || i == hover_idx)
        {
            ...
            DrawRectangleRoundedAlign(right_x - rect_pad, hitbox[i].y + hitbox[i].height * 0.5f, rect_w, rect_w, 1.f, 4, theme_map[text[i]].text, RIGHT, CENTER);
            DrawRectangleRoundedAlign(right_x - rect_w - 2*rect_pad, hitbox[i].y + hitbox[i].height * 0.5f, rect_w, rect_w, 1.f, 4, theme_map[text[i]].sub, RIGHT, CENTER);
            DrawRectangleRoundedAlign(right_x - 2*rect_w - 3*rect_pad, hitbox[i].y + hitbox[i].height * 0.5f, rect_w, rect_w, 1.f, 4, theme_map[text[i]].main, RIGHT, CENTER);
        }
